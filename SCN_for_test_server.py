...
if __name__ == '__main__':
    ...
    data = scipy.io.loadmat('./data/coco/resnet_feats_test.mat')
    img_feats = data['feats']

    data = scipy.io.loadmat('./data/coco/tag_feats_test.mat')
    tag_feats = data['feats']
        
    z = img_feats.T.astype('float64')
    y = tag_feats.T.astype('float64')
    ...
    cPickle.dump(N_best_list, open("coco_nbest_server.p", "wb"))
    ...
    open('./coco_scn_candidates_server.txt', 'w').write('\n'.join([cap[0] for cap in predtext]))
    
    """ store generated captions.  """
    x = cPickle.load(open("./data/coco/test_server_info.p","rb"))
    test_info, val_info = x[0],x[1]
    del x
    
    captions = []
    for i in range(len(predtext)):
        tmp = {}
        tmp["caption"] = predtext[i][0]
        tmp["image_id"] = test_info[i][1]
        captions.append(tmp)
    
    json.dump(captions, open('captions_test2014_results.json', 'w'))
